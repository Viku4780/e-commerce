...
const Checkout = () => {
    ...
    // const { addCheckout } = useSelector(state => state.checkout);
    ...
    const handleCreateCheckout = async (e) => {
        ...
        if (cart && cart.products.length > 0) {
            ...
            if (res.payload.order && res.payload.order.id) {
                setCheckoutId(res.payload.newCheckout._id); // set checkout ID if checkout was successful
                setOrderId(res.payload.order.id);
                dispatch(addCheckout(res.payload.newCheckout))
            }
        }
    };
    ...
    const handlePaymentSuccess = async (details) => {
        try {
            const response = await axios.put(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/checkout/pay`, { paymentStatus: "paid", paymentDetails: details },
                {
                    headers: {
                    ...
                    <div className='mt-6'>
                        {!checkoutId ? (
                            ...
                            <div>
                                ...
                                <RazorPayButton amount={cart.totalPrice} order_id={orderId} checkoutId={checkoutId} />
                            </div>
                        )}
                    </div>
                </form>
                    ...
                    {cart.products.map((product, index) => (
                        <div key={index} className='flex items-start justify-between py-2 border-b'>
                            <div className='flex items-start'>
                                <Image width={100} height={100} src={product.image} alt={product.name} className='w-20 h-24 object-cover mr-4' />
