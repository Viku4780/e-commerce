    ...
    const handleImage = async (e) => {
        ...
        try {
            ...
            const { data } = await axios.post(
                `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/upload`, formData,
                {
                    headers: {
        ...
        <div className='max-w-5xl mx-auto p-6 shadow-md rounded-md '>
            ...
            <form onSubmit={handleSubmit}>
                ...
                <div className='mb-6'>
                    ...
                    <input type="text" name="name" value={productData.name} onChange={handleChange} className='w-full border border-gray-300 rounded-md p-2'
                        required
                    />
                </div>
                ...
                <div className='mb-6'>
                    ...
                    <div className='flex gap-4 mt-4'>
                        {productData.images.map((image, index) => (
                            <div key={index}>
                                <Image width={100} height={100} src={image.url} alt={image.altText || "Product Image"} className='w-20 h-20 object-cover rounded-md shadow-md' />
                            </div>
                        ))}
                    </div>
                </div>

                <button type='submit' className='w-full bg-green-500 text-white py-2 rounded-md hover:bg-green-600 transition-colors bg-black'>Update Product</button>
            </form>
        </div>
    )
}
