  ...
  return (
    <div className='max-w-7xl mx-auto p-4 sm:p-6'>
      ...
      {!orderDetails ? (<p>No Order details found</p>) : (<div className='p-4 sm:p-6 rounded-lg border'>
              ...
              {orderDetails.orderItems.map((item) => (
                <tr key={item.productId} className='border-b'>
                  <td className='py-2 px-4 flex items-center'>
                    <Image width={50} height={50} src={item.image} alt='item.name' className='w-12 h-12 object-cover rounded-lg mr-4' />
        ...
        {/* back to orders link */}
        <Link href={'/my-orders'} className='text-blue-500 hover:underline'>Back to My Orders</Link>
      </div>
    )}
    </div>
  )
